...
BEGIN
    ...
    IF p_product_id IS NULL THEN
        INSERT INTO Products (category_id, title, description, brand, is_active, created_at)
        VALUES (p_category_id, p_title, p_description, p_brand, TRUE, NOW());
    ...
    ELSE
        ...
        DELETE FROM ProductImages WHERE product_id = v_product_id;
    END IF;
    ...
    IF p_image_url IS NOT NULL AND p_image_url != '' THEN
        ...
        VALUES (v_product_id, p_image_url, p_image_alt_text, p_image_sort_order);
    END IF;
...
-- Create new product with one image
CALL sp_ManageProduct(
    NULL,
    31,
    'Beginner Guitar',
    'Entry-level acoustic guitar for students',
    'Yamaha',
    'https://cdn.urbanease.com/images/beginner-guitar.jpg',
    'Front View',
    0
);

-- Update existing product (e.g., ID = 64) with new image
CALL sp_ManageProduct(
    64,
    31,
    'Updated Guitar',
